...
class TrackMatchingUseCase @Inject constructor() {
    fun ranked(target: TrackIdentity, candidates: List<SourceCandidate>): List<SourceCandidate> {
        ...
        if (target.album.isNullOrBlank() && (target.durationSeconds ?: 0) <= 0 &&
            tied.map { normalize(it.first.identity.album.orEmpty()) }.filter(String::isNotEmpty).distinct().size > 1) return emptyList()
    ...
    private fun score(wanted: TrackIdentity, found: TrackIdentity): Int? {
        ...
        val expectedDuration = wanted.durationSeconds?.takeIf { it > 0 }
        val actualDuration = found.durationSeconds?.takeIf { it > 0 }
        val difference = if (expectedDuration != null && actualDuration != null) {
            abs(expectedDuration - actualDuration).also { if (it > 5) return null }
        } else null
